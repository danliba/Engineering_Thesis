...
for k=1:length(C)
    ...
    vflot.(my_field) = data_reg2;
end

%% ahora guardamos

save('Flotos_tesis_fis.mat','-struct','vflot');
save('Flotos_tesis_locat.mat','-struct','flot');
save('N_flotos.mat','C');
%%
% for ix=1:1:length(flot_values)
%     if ~isempty(flot_values{ix, 1})==0 %si esta vacio, continua
%         continue
%     end
%     
%     if size(flot_values{ix, 1}{1,1},2)>1
%         disp([num2str(ix),' Se parte'])
%         
%         for it=1:1:size(flot_values{ix, 1}{1,1},2)
%             flot_values{275, 1}{1, 1}(:,1)
%         
%         
%         end 
%         
%     else
%         disp([num2str(ix),' No se parte'])
%     end
% end
%%
%si el size de temp es mayor que 1
%entonces split 

% if size(temp1,2)>1
%     disp('Se parte')
%     for ix=1:1:size(temp1,2)
%         tempdata{ix,:}=temp1(1:50,ix);
%         saldata{ix,:}=sal1(1:50,ix);
%         depthdata{ix,:}=depth1(1:50,ix);
%         
%         data_p_mat=[tempdata(ix),saldata(ix),depthdata(ix)];
%         data_mat(:,:,ix)=cell2mat(data_p_mat);
%         
%     end
% end
