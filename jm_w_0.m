%% Su principal zona de desove se ubica entre los 14º00' y
%los 18º30' S de nuestro litoral, en el frente oceánico 
%limitado por las ACF de intenso afloramiento y las ASS 
...
pathin='F:\';
% output file path
pathout='F:\simulaciones\prueba2';
% Output file name
outfn=sprintf('OffPeruJackMac_%04d%02d%02d_%04d%02d%02d.nc',...
    styear,startmon,startday,enyear,endmon,endday);
...
if plotflag==1
    figure
    %aviobj = QTWriter('Jack_mack2.mov','FrameRate',5); 
end
...
for i=1:1:numiter
    ...
    % reading time vector of the input
    timein=ncread(ncfnfull,'time');
    
    %yydd=find(timein>timenow);%si ultimo dia del año no cumple la condicion 
    ...
    if ~isempty(id0)
        ...
        %añadido para ultimo dia del año
%         if isempty(yydd)
%             id2=min(find(timein+1>timenow));
%             disp(id2)
%             time1=timein(id1);
%             time2=timein(id2);
%             dT=time2-time1;
...
%         else
        ...
        if id1==id1p & id2==id2p
            ...
            if isempty(id2)%para ultimo dia se deja las var estatica
                dui=ui1;
                dvi=vi1;
                dwi=wi1;
                dtempi=tempi1;
